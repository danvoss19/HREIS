...
<head>
  ...
  <style>
    ...
    * {
    ...
    .department-card .member {
      overflow: hidden;
      text-overflow: ellipsis;
      white-space: nowrap;
    }

    .department-card .members-container {
      flex: 1;
      overflow-y: auto;
      margin-top: 10px;
    }
...
<body>
  ...
  $sql = "SELECT department, id, firstname, lastname, avatar 
        FROM employees 
  ...
  <div class="main">
    ...
    <div class="departments" id="departmentsList">
      <?php foreach ($departments as $deptName => $members):
        $memberCount = count($members);
        $displayMembers = array_slice($members, 0, 5);
      ?>
        <div class="department-card">
          <h3><?= htmlspecialchars($deptName) ?>
            <small>- <?= $memberCount ?> Members</small>
          </h3>
          <a href="department_users.php?department=<?= urlencode($deptName) ?>" class="view-all" style="background-color: #7152F3;">View All</a>
          ...
          <div class="members-container">
            <?php foreach ($displayMembers as $m): ?>
              <div class="member">
                ...
                <img src="<?= htmlspecialchars($avatar) ?>"
                  alt="<?= htmlspecialchars($m['firstname'] . ' ' . $m['lastname']) ?>">
            ...
            <?php if ($memberCount > 5): ?>
              <div class="member">
                <span>+ <?= $memberCount - 5 ?> more members</span>
              </div>
            <?php endif; ?>
          </div>
        </div>
